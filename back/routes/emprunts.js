...
router.get("/livres", async (req, res) => {
  try {
    ...
    const [total] = await db
      .promise()
      .query("SELECT COUNT(*) as count FROM livre");
    const totalBooks = total[0].count;

    const [books] = await db
      .promise()
      .query("SELECT * FROM livre LIMIT ? OFFSET ?", [limit, offset]);
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const [livre] = await db
      .promise()
      .query("SELECT quantite FROM livre WHERE id = ?", [livre_id]);
    if (!livre[0])
      return res.status(404).json({ message: "Livre introuvable" });
    if (livre[0].quantite <= 0)
    ...
    await db
      .promise()
      .query("UPDATE livre SET quantite = quantite - 1 WHERE id = ?", [
        livre_id,
      ]);
